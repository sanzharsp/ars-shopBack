...
class OrderedViewAPI(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = Order.objects.create(
                            ...
                            productDetail =request.data['productDetail'],
                            post_index =serializer.data['post_index'], 
                            telephone =serializer.data['telephone'], 
                            totalPrice =serializer.data['totalPrice'], 
                            )
        my_related_objs = request.data['products']
      

   
        for i in range(0, len(my_related_objs)):
            data.products.add(News.objects.get(id=my_related_objs[i]))
