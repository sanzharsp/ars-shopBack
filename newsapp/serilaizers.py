...
class AuthorizateSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        data['user'] =AuthorSerilizer(Author.objects.get(username=user.get_username())).data
        return data
...
class Last_News_Serilizer(serializers.ModelSerializer):
    ...
    class Meta:
        model= News
        fields = ('id','title','image1','category','user','context','date_add','published','likes','value', 'qty', 'price',)
...
class ProductsSerilizerID(serializers.ModelSerializer):
    post = serializers.IntegerField(
        label=("Id продукта"),
        write_only=True)
    class Meta:
...
class OrderSerilizer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = 'address','post_index','telephone' ,'totalPrice'
